...
class LSTMAgent():
    def __init__(self, 
        ...
        self.nb_actions = 5 * self.nb_agent
        self.nb_obs = 231 * self.nb_agent
        self.nb_inputs = self.nb_actions + self.nb_obs + (2 + 1) * self.nb_agent + 1
    ...
    def prepare_input(self, next_obs, agent_id, action_onehot, reward, timestep):
        obs = normalize_observation(next_obs[0], tree_depth=2, observation_radius=10)
        timetable = np.array([self.env.agents[agent_id].earliest_departure, self.env.agents[agent_id].latest_arrival])
        
        return np.concatenate((obs, action_onehot, timetable, np.array([reward,timestep]))).reshape(1,self.nb_inputs)
    ...
    def LSTM_Model(self):
        ...
        action = layers.Dense(self.nb_actions)(common)
    ...
    def train(self):
        ...
        for episode in range(self.nb_episodes):
            ...
            with tf.GradientTape() as tape:
                ...
                rewards_history = []
                reward = 0.0
                action_onehot = np.zeros((self.nb_agent ,self.nb_actions))
                ...
                while True:
                    ...
                    action_probs = tf.reshape(action_probs, (self.nb_agent, self.nb_actions))
                    action_probs = sp.special.softmax(action_probs, axis=1)
                    
                    actions = {}
                    action_onehot = np.zeros((self.nb_agent ,self.nb_actions))
                    for i in range(self.nb_agent):
                        actions[i] = np.random.choice(self.nb_actions, p=action_probs[i].numpy())
                        entropy += sp.stats.entropy(action_probs[i])
                        action_onehot[i][actions] = 1.0
                    action_probs_history += action_probs.flatten().numpy().tolist()
                    action_distribution += actions.items()
